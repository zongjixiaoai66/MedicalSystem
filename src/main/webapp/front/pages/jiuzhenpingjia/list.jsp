  ...
  <div id="app">
    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="taidupingfen" id="taidupingfen" placeholder="态度评分" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('jiuzhenpingjia','新增')" @click="jump('../jiuzhenpingjia/add.jsp')" type="button" class="layui-btn btn-theme">
    ...
    <div class="product">
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        ...
        <div class="proBoxtxt">
          <a :href="'../jiuzhenpingjia/detail.jsp?id='+item.id">查看详情</a>
        </div>
      </div>
      <div class="clear"></div>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#taidupingfen').val()) {
          param['taidupingfen'] = jquery('#taidupingfen').val() ? '%' + jquery('#taidupingfen').val() + '%' : '';
        }  
        if (jquery('#yonghuming').val()) {
          param['yonghuming'] = jquery('#yonghuming').val() ? '%' + jquery('#yonghuming').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('jiuzhenpingjia/list', 'get', param, function(res) {
