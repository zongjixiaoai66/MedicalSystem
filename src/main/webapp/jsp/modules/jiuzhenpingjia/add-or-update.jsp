		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑就诊评价</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>就诊评价管理</span></li>
								<li class="breadcrumb-item active"><span>编辑就诊评价</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">就诊评价信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>出诊时间</label>
										<input id="chuzhenshijian" name="chuzhenshijian" class="form-control" placeholder="出诊时间">
									</div>
									<div class="form-group col-md-6">
										<label>时间段</label>
									...
									<div class="form-group col-md-6">
										<label>态度评分</label>
										<select id="taidupingfenSelect" name="taidupingfen" class="form-control">
												<option value=""></option>
												<option class="taidupingfenOption" value="1">
													1
												</option>
												<option class="taidupingfenOption" value="2">
													2
												</option>
												<option class="taidupingfenOption" value="3">
													3
												</option>
												<option class="taidupingfenOption" value="4">
													4
												</option>
												<option class="taidupingfenOption" value="5">
													5
												</option>
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>技术评分</label>
										<select id="jishupingfenSelect" name="jishupingfen" class="form-control">
												<option value=""></option>
												<option class="jishupingfenOption" value="1">
													1
												</option>
												<option class="jishupingfenOption" value="2">
													2
												</option>
												<option class="jishupingfenOption" value="3">
													3
												</option>
												<option class="jishupingfenOption" value="4">
													4
												</option>
												<option class="jishupingfenOption" value="5">
													5
												</option>
										</select>	
									</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "jiuzhenpingjia/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("jiuzhenpingjia/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("jiuzhenpingjia/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("jiuzhenpingjia"+colName,"GET",{
		...
		function dataBind(){
			...
			$("#chuzhenshijian").val(ruleForm.chuzhenshijian);	
			$("#shijianduan").val(ruleForm.shijianduan);	
			var taidupingfenOptions = document.getElementsByClassName("taidupingfenOption");
			for(var taidupingfenCount = 0; taidupingfenCount < taidupingfenOptions.length;taidupingfenCount++){
				if(taidupingfenOptions[taidupingfenCount].getAttribute('value') == ruleForm.taidupingfen){
					taidupingfenOptions[taidupingfenCount].setAttribute('selected','selected');
				}
			}
			var jishupingfenOptions = document.getElementsByClassName("jishupingfenOption");
			for(var jishupingfenCount = 0; jishupingfenCount < jishupingfenOptions.length;jishupingfenCount++){
				if(jishupingfenOptions[jishupingfenCount].getAttribute('value') == ruleForm.jishupingfen){
					jishupingfenOptions[jishupingfenCount].setAttribute('selected','selected');
				}
			}
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					$("#chuzhenshijian").val(res.data.chuzhenshijian);
					}
					if(res.data != null && res.data != '' && res.data.shijianduan != null && res.data.shijianduan != ''){

					$("#shijianduan").val(res.data.shijianduan);
					}
					if(res.data != null && res.data != '' && res.data.taidupingfen != null && res.data.taidupingfen != ''){

					var taidupingfenOptions = document.getElementsByClassName("taidupingfenOption");
						for(var taidupingfenCount = 0; taidupingfenCount < taidupingfenOptions.length;taidupingfenCount++){
							if(taidupingfenOptions[taidupingfenCount].getAttribute('value') == res.data.taidupingfen){
								taidupingfenOptions[taidupingfenCount].setAttribute('selected','selected');
							}
						}
					}
					if(res.data != null && res.data != '' && res.data.jishupingfen != null && res.data.jishupingfen != ''){

					var jishupingfenOptions = document.getElementsByClassName("jishupingfenOption");
						for(var jishupingfenCount = 0; jishupingfenCount < jishupingfenOptions.length;jishupingfenCount++){
							if(jishupingfenOptions[jishupingfenCount].getAttribute('value') == res.data.jishupingfen){
								jishupingfenOptions[jishupingfenCount].setAttribute('selected','selected');
							}
						}
					}
